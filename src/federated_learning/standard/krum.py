...
from torch import Tensor, cat, stack, cdist
...
class Server(BaseServer): 
    ...
    def _krum_function(
        ...
        # Models distances (Distances between (W - dW) and W are equivalent)
        W = stack([cat([p.view(-1) for p in update["model"]]) for update in updates])
        rank = cdist(W, W).sum(0).argsort().tolist()
        return [updates[i]["model"] for i in rank[:self.K]]
    ...
    def aggregation(
        ...
        # Krum function
        models = self._krum_function(updates)
        ...
        for model in models:
            for p_new, p in zip(model, self.model.parameters()):
                p.data.add_(p_new / self.K)
